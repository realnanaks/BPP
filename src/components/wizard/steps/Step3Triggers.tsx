'use client';
import { Layers, Zap, Users, Code, Trash2, Plus, X, ChevronDown, CheckCircle2, Save, AlertTriangle, Copy } from 'lucide-react';
        ...
        events: [
            ...
            },
            {
                ...
                params: [
                    ...
                    { name: 'Outcome', type: 'enum', options: ['Win', 'Loss', 'One-Cut'] }
                ]
            },
            {
...
export default function StepTriggers() {
    ...
    // Initial load: Add empty Deposit trigger if none exists
    useEffect(() => {
    ...
    return (
        <div className="step-container">
                    ...
                    <div className="panel-header">
                        <div className="header-row">
                            <div>
                                ...
                                <p className="panel-subtitle">Define the event-driven triggers for this promotion.</p>
                            </div>
                            <div className="sh-actions">
                                <button className={`btn-xs ${isAdvancedMode ? 'active-mode' : ''}`} onClick={() => setIsAdvancedMode(!isAdvancedMode)}>
                                    <Code size={12} /> Advanced Mode
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <style jsx>{`
                .step-container { max-width: 900px; margin: 0 auto; animation: fadeUp 0.3s ease; }
                @keyframes fadeUp { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }

                .trigger-layout { display: flex; flex-direction: column; gap: 24px; min-height: 500px; }
                .glass-panel { background: var(--color-bg-panel); border: 1px solid var(--color-border); border-radius: 12px; padding: 24px; flex: 1; display: flex; flex-direction: column; }
                
                .form-scrollable { overflow-y: auto; flex: 1; scrollbar-width: thin; scrollbar-color: var(--color-border) transparent; margin-top: 16px; }
                ...
                .panel-title { color: var(--color-text-primary); font-size: 18px; margin: 0 0 4px; display: flex; align-items: center; gap: 8px; }
                .panel-subtitle { color: var(--color-text-muted); font-size: 13px; margin: 0; }
                .header-row { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 1px solid var(--color-border); padding-bottom: 16px; }

                .text-yellow { color: var(--color-betika-yellow); }

                /* Trigger Cards */
                .triggers-container { display: flex; flex-direction: column; gap: 16px; padding-bottom: 24px; }
                ...
                .param-selector-inline { background: var(--color-bg-panel); border: 1px solid var(--color-border); border-radius: 6px; width: fit-content; min-width: 200px; animation: fadeIn 0.1s; z-index: 10; position: relative; }
